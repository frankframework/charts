...
# ff-common: The Helm Helper Chart
...
A few tips for working with ff-common:

- Be careful when using functions that generate random data (like `ff-common.fullname.unique`).
...
We define some of the most ff-common kinds in a way that lets you easily work with
them.
...
```yaml
{{- template "ff-common.service" (list . "mychart.service") -}}
...
Note that the `ff-common.service` template defines two parameters:
...
The `ff-common.service` template is responsible for rendering the templates with
...
The `ff-common.service` template creates a basic `Service` resource with the
following defaults:
...
- Selector set to `app: {{ template "ff-common.name" }}, release: {{ .Release.Name | quote }}` to match the default used in the `Deployment` resource

Example template:

```yaml
{{- template "ff-common.service" (list . "mychart.mail.service") -}}
{{- define "mychart.mail.service" -}}
metadata:
  name: {{ template "ff-common.fullname" . }}-mail # overrides the default name to add a suffix
...
{{- end -}}
---
{{ template "ff-common.service" (list . "mychart.web.service") -}}
{{- define "mychart.web.service" -}}
metadata:
  name: {{ template "ff-common.fullname" . }}-www  # overrides the default name to add a suffix
  labels:                                       # appended to the labels section
    protocol: www
spec:
  ports:                                        # composes the `ports` section of the service definition.
...
## `ff-common.deployment`

The `ff-common.deployment` template defines a basic `Deployment`. Underneath the
hood, it uses `ff-common.container` (see next section).

By default, the pod template within the deployment defines the labels `app: {{ template "ff-common.name" . }}`
...
```yaml
{{- template "ff-common.deployment" (list . "mychart.deployment") -}}
...
The `ff-common.container` template creates a basic `Container` spec to be used
...
spec:
  template:
    spec:
      containers:
      - {{ template "ff-common.container" (list . "mychart.deployment.container") }}
{{- end -}}
...
`ff-common.container` template to populate the PodSpec's container list. The usage
...
The `common.chart` helper prints the chart name and version, escaped to be
